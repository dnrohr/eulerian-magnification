...
The analyzer can be rebound with pulse, breathing, or fast-motion bands. Pulse uses 0.7-3.0 Hz, breathing uses 0.1-0.6 Hz, and Fast Motion uses 4.0-12.0 Hz. The UI exposes these presets along with an amplification slider. Recording metadata and the debug MP4 overlay preserve the selected mode and band, so early captures can be traced back to their filter settings. Latency is estimated from the frame timestamp and shown next to analysis FPS. A timestamp tracker flags repeated or decreasing timestamps.
...
- Pulse, breathing, and fast-motion presets are explicit because each mode needs different temporal bands and user expectations. Earlier overlapping Tremor/Object UI presets are collapsed into Fast Motion until the app has distinct motion rendering behavior for them.
