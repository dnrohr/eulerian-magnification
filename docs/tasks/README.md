...
| M | [MILESTONE_M_ROI_COORDINATE_MAPPING.md](MILESTONE_M_ROI_COORDINATE_MAPPING.md) | Complete |
...
command templates without the surrounding checklist. Setup commands that assert
a visible target and final visual-acceptance commands are commented by default
in `-CommandsOnly` output because they must not be run unattended; pass
...
placeholder command. In an all-stage ROI handoff, the runbook and Markdown
handoff list setup and non-placeholder commands separately from placeholder ROI
final templates, which are commented as reference-only so they are not pasted
directly.
