...
Assert-True -Condition ($result.setupReadiness.command.Contains("export_pixel_session_readiness.ps1")) -Message "Handoff should expose a setup readiness snapshot command."
Assert-True -Condition ($result.setupReadiness.command.Contains('-DeviceSerial "PIXEL-HANDOFF-TEST"')) -Message "Handoff setup readiness command should preserve device serial."
Assert-True -Condition ($result.setupReadiness.command.Contains("-FailOnSetupNotReady")) -Message "Handoff setup readiness command should fail when setup is not ready."
Assert-True -Condition ($result.setupReadiness.outputPath.Contains("pixel_setup_readiness_preflight.json")) -Message "Handoff should report the setup readiness output path."
Assert-True -Condition ($result.setupReadiness.note.Contains("readyForSetupCapture")) -Message "Handoff should explain setup-capture readiness."
Assert-True -Condition ($result.sessionReadiness.command.Contains("export_pixel_session_readiness.ps1")) -Message "Handoff should expose a session readiness snapshot command."
Assert-True -Condition ($result.sessionReadiness.command.Contains('-DeviceSerial "PIXEL-HANDOFF-TEST"')) -Message "Handoff session readiness command should preserve device serial."
Assert-True -Condition ($result.sessionReadiness.command.Contains("-FailOnNotReady")) -Message "Handoff session readiness command should fail when the device is not ready."
Assert-True -Condition ($result.sessionReadiness.outputPath.Contains("pixel_session_readiness_preflight.json")) -Message "Handoff should report the session readiness output path."
Assert-True -Condition ($result.sessionReadiness.note.Contains("readyForWatchedCapture")) -Message "Handoff should explain watched-capture readiness."
Assert-True -Condition ($result.sessionReadiness.note.Contains("readyForSetupCapture")) -Message "Handoff should explain setup readiness."
Assert-True -Condition ($result.sessionReadiness.note.Contains("cameraLogHealth")) -Message "Handoff should explain camera log health."
Assert-True -Condition ($result.sessionReadiness.note.Contains("recommendedActions")) -Message "Handoff should explain readiness actions."
Assert-True -Condition ($result.previewRecovery.command.Contains("recover_pixel_preview_session.ps1")) -Message "Handoff should expose a preview recovery command."
Assert-True -Condition ($result.previewRecovery.command.Contains('-DeviceSerial "PIXEL-HANDOFF-TEST"')) -Message "Handoff preview recovery command should preserve device serial."
Assert-True -Condition ($result.previewRecovery.command.Contains("-WaitForThermalReady")) -Message "Handoff preview recovery command should wait for thermal readiness."
Assert-True -Condition ($result.previewRecovery.command.Contains("-ThermalReadyBelowStatus 3")) -Message "Handoff preview recovery should wait below severe thermal status."
Assert-True -Condition ($result.previewRecovery.note.Contains("camera frame-sync warnings")) -Message "Handoff should explain when to run preview recovery."
...
Assert-True -Condition ($runbook.Contains("Snapshot setup readiness")) -Message "Runbook should label the setup readiness step."
Assert-True -Condition ($runbook.Contains("pixel_setup_readiness_preflight.json")) -Message "Runbook should include the setup readiness output path."
Assert-True -Condition ($runbook.Contains("-FailOnSetupNotReady")) -Message "Runbook should use the setup readiness failure gate."
Assert-True -Condition ($runbook.Contains("export_pixel_session_readiness.ps1")) -Message "Runbook should include the session readiness command."
Assert-True -Condition ($runbook.Contains("Snapshot session readiness")) -Message "Runbook should label the session readiness step."
Assert-True -Condition ($runbook.Contains("Recover a frozen Pixel camera preview")) -Message "Runbook should label the preview recovery step."
Assert-True -Condition ($runbook.Contains("recover_pixel_preview_session.ps1")) -Message "Runbook should include the preview recovery command."
...
Assert-True -Condition ($handoff.Contains("Setup readiness command:")) -Message "Markdown handoff should include the setup readiness command summary."
Assert-True -Condition ($handoff.Contains("pixel_setup_readiness_preflight.json")) -Message "Markdown handoff should include the setup readiness output path."
Assert-True -Condition ($handoff.Contains("-FailOnSetupNotReady")) -Message "Markdown handoff should use the setup readiness failure gate."
Assert-True -Condition ($handoff.Contains("Session readiness command:")) -Message "Markdown handoff should include the session readiness command summary."
Assert-True -Condition ($handoff.Contains("export_pixel_session_readiness.ps1")) -Message "Markdown handoff should include the session readiness helper."
Assert-True -Condition ($handoff.Contains("nearly frozen preview")) -Message "Markdown handoff should explain the frozen-preview readiness check."
Assert-True -Condition ($handoff.Contains("readyForWatchedCapture")) -Message "Markdown handoff should explain watched-capture readiness."
Assert-True -Condition ($handoff.Contains("readyForSetupCapture")) -Message "Markdown handoff should explain setup readiness."
Assert-True -Condition ($handoff.Contains("cameraLogHealth")) -Message "Markdown handoff should explain camera log health."
Assert-True -Condition ($handoff.Contains("recommendedActions")) -Message "Markdown handoff should explain readiness actions."
...
Assert-True -Condition ($manifest.setupReadiness.command.Contains("export_pixel_session_readiness.ps1")) -Message "Manifest should include setup readiness command metadata."
Assert-True -Condition ($manifest.setupReadiness.command.Contains("-FailOnSetupNotReady")) -Message "Manifest setup readiness command should fail when setup is blocked."
Assert-True -Condition ($manifest.setupReadiness.outputPath.Contains("pixel_setup_readiness_preflight.json")) -Message "Manifest should include setup readiness output path."
Assert-True -Condition ($manifest.setupReadiness.note.Contains("readyForSetupCapture")) -Message "Manifest should explain setup readiness."
Assert-True -Condition ($manifest.sessionReadiness.command.Contains("export_pixel_session_readiness.ps1")) -Message "Manifest should include session readiness command metadata."
Assert-True -Condition ($manifest.sessionReadiness.outputPath.Contains("pixel_session_readiness_preflight.json")) -Message "Manifest should include session readiness output path."
Assert-True -Condition ($manifest.sessionReadiness.note.Contains("readyForWatchedCapture")) -Message "Manifest should explain watched-capture readiness."
Assert-True -Condition ($manifest.sessionReadiness.note.Contains("readyForSetupCapture")) -Message "Manifest should explain setup readiness."
Assert-True -Condition ($manifest.sessionReadiness.note.Contains("cameraLogHealth")) -Message "Manifest should explain camera log health."
...
Assert-True -Condition ($roiRunbook.Contains("manual-roi-known-target")) -Message "ROI runbook should still include the setup capture command."
Assert-True -Condition ($roiRunbook.Contains("# 3b. ROI final templates below are reference only; paste the helper output instead.")) -Message "ROI runbook should mark placeholder finals as reference-only."
Assert-True -Condition ($roiRunbook.Contains("# TEMPLATE ONLY: .\tools\capture_live_validation_evidence.ps1")) -Message "ROI runbook should comment out placeholder final templates."
Assert-True -Condition ($roiRunbook.IndexOf("manual-roi-known-target") -lt $roiRunbook.IndexOf("prepare_roi_final_capture_command.ps1")) -Message "ROI runbook should list setup capture before helper."
Assert-True -Condition ($roiRunbook.IndexOf("prepare_roi_final_capture_command.ps1") -lt $roiRunbook.IndexOf("# TEMPLATE ONLY:")) -Message "ROI runbook should list helper before placeholder templates."
...
Assert-True -Condition (($textOutput -join "`n").Contains("Setup readiness command:")) -Message "Text handoff should print the setup readiness command."
Assert-True -Condition (($textOutput -join "`n").Contains("Session readiness command:")) -Message "Text handoff should print the session readiness command."
Assert-True -Condition (($textOutput -join "`n").Contains("Preview recovery command:")) -Message "Text handoff should print the preview recovery command."
