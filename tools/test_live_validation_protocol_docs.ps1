...
foreach ($path in @($readme, $taskReadme)) {
    ...
    Assert-DocContains -Path $path -Expected "-FailOnDeviceUnavailable" -Message "Operator docs must document device-availability handoff failure mode."
    Assert-DocContains -Path $path -Expected "expected Pixel serial is connected" -Message "Operator docs must document handoff device availability metadata."
    Assert-DocContains -Path $path -Expected "install_debug_on_pixel.ps1 -Build -Launch" -Message "Operator docs must document the handoff install command."
    ...
    Assert-DocContains -Path $path -Expected "pixel_validation_handoff_manifest.json" -Message "Operator docs must document the handoff manifest artifact name."
    ...
    Assert-DocContains -Path $path -Expected "pixel_setup_readiness_preflight.json" -Message "Operator docs must document the setup readiness output artifact."
    Assert-DocContains -Path $path -Expected "-FailOnSetupNotReady" -Message "Operator docs must document the setup readiness failure mode."
    ...
    Assert-DocContains -Path $path -Expected "live_validation_review_queue.json" -Message "Operator docs must document the review queue handoff artifact name."
    ...
    Assert-DocContains -Path $path -Expected "-FailOnMissingVisualReviewText" -Message "Operator docs must document visual-review text closeout gate."
    Assert-DocContains -Path $path -Expected "-FailOnReviewContactSheetIssues" -Message "Operator docs must document review contact sheet closeout gate."
    ...
    Assert-DocContains -Path $path -Expected "review contact sheet" -Message "Operator docs must document review contact sheet hashes."
...
Assert-DocContains -Path $liveGuide -Expected "TargetDescription" -Message "Live guide must document final target description."
Assert-DocContains -Path $liveGuide -Expected "VisualClaim" -Message "Live guide must document final visual claim."
...
Assert-DocContains -Path $parityDoc -Expected "-FailOnReviewContactSheetIssues" -Message "Parity targets must document review contact sheet accepted evidence gate."
