    ...
    [pscustomobject]@{
        ...
        commands = @(
            [pscustomobject]@{
                ...
                command = "Run .\tools\summarize_pixel_validation_closeout.ps1 -FailOnCloseoutNotReady and .\tools\summarize_pixel_validation_closeout.ps1 -FailOnPresetDocsNotReady. If both pass, update README.md and docs/testing/MIT_PARITY_TARGETS.md with visual-validation status and artifact notes, then rerun .\tools\test_offline_project_tooling.ps1."
            }
        )
    }
)
