...
Assert-Equal -Actual $plan.closeoutBlockerCount -Expected 1 -Message "Plan should expose a compact closeout blocker count."
...
Assert-Equal -Actual $plan.recommendedCaptureCount -Expected 6 -Message "Plan should expose a compact recommended capture count."
...
Assert-Equal -Actual $plan.recommendedCaptures[0].commands[0].operatorRequired -Expected $true -Message "Setup command should expose operator-required metadata."
Assert-Equal -Actual $plan.recommendedCaptures[0].commands[0].finalVisualAcceptanceRequired -Expected $false -Message "Setup command should not require final visual acceptance."
...
Assert-Equal -Actual $plan.recommendedCaptures[0].commands[1].operatorRequired -Expected $true -Message "Final command should expose operator-required metadata."
Assert-Equal -Actual $plan.recommendedCaptures[0].commands[1].finalVisualAcceptanceRequired -Expected $true -Message "Final command should expose visual-acceptance metadata."
Assert-True -Condition (($plan.recommendedCaptures[0].operatorSetup -join "`n").Contains("high-contrast target")) -Message "Recommended manual ROI capture should include operator setup guidance."
Assert-True -Condition (($plan.recommendedCaptures[0].operatorSetup -join "`n").Contains("convert_roi_bounds_to_normalized.ps1")) -Message "Recommended manual ROI capture should mention the ROI bounds converter."
Assert-True -Condition (($plan.recommendedCaptures[0].operatorSetup -join "`n").Contains("MeasureRoiExpected")) -Message "Recommended manual ROI capture should mention the paste-ready ROI value."
Assert-True -Condition (($plan.recommendedCaptures[0].acceptanceChecks -join "`n").Contains("Exactly one manual ROI outline")) -Message "Recommended manual ROI capture should include acceptance checks."
Assert-True -Condition (($plan.recommendedCaptures[1].operatorSetup -join "`n").Contains("convert_roi_bounds_to_normalized.ps1")) -Message "Recommended automatic ROI capture should mention the ROI bounds converter."
Assert-True -Condition (($plan.recommendedCaptures[1].operatorSetup -join "`n").Contains("MeasureRoiExpected")) -Message "Recommended automatic ROI capture should mention the paste-ready ROI value."
...
Assert-True -Condition (($planText -join "`n").Contains("Recommended captures: 6")) -Message "Text plan should print recommended capture count."
...
Assert-True -Condition (($planText -join "`n").Contains("Setup: Use a stable, high-contrast target")) -Message "Text plan should print operator setup guidance."
Assert-True -Condition (($planText -join "`n").Contains("convert_roi_bounds_to_normalized.ps1")) -Message "Text plan should print ROI converter guidance."
Assert-True -Condition (($planText -join "`n").Contains("Accept: Exactly one manual ROI outline")) -Message "Text plan should print acceptance checks."
...
Assert-True -Condition (($pulseOnlyPlan.recommendedCaptures[0].acceptanceChecks -join "`n").Contains("not an ROI-only tint")) -Message "Pulse slot guidance should call out ROI-only tint rejection."
