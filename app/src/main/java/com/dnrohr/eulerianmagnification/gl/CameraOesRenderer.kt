...
class CameraOesRenderer(
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        phaseRieszProgram = GlProgram.compileProgram(
            RieszPhaseShaderSource.VERTEX,
            RieszPhaseShaderSource.RIESZ_COMPONENT_FRAGMENT,
        )
        phaseProjectProgram = GlProgram.compileProgram(
            RieszPhaseShaderSource.VERTEX,
    ...
    private fun renderLivePhaseRieszComponents(state: LivePhaseRoiState) {
        state.rieszComponents.bind()
        GLES30.glUseProgram(phaseRieszProgram)
        GLES30.glUniform1i(phaseRieszInputTextureLocation, 0)
        GLES30.glUniform2f(
            phaseRieszTexelSizeLocation,
            1.0f / state.extractedRoi.size.width,
            1.0f / state.extractedRoi.size.height,
        )
        GLES30.glActiveTexture(GLES30.GL_TEXTURE0)
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, state.extractedRoi.textureId)
        drawFramebufferQuad()
        GLES30.glBindFramebuffer(GLES30.GL_FRAMEBUFFER, 0)
        GlProgram.checkNoGlError("renderLivePhaseRieszComponents")
    }

    private fun renderLivePhaseProjection(state: LivePhaseRoiState) {
        state.projectedPhase.bind()
        GLES30.glUseProgram(phaseProjectProgram)
        GLES30.glUniform1i(phaseProjectRieszTextureLocation, 0)
        GLES30.glActiveTexture(GLES30.GL_TEXTURE0)
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, state.rieszComponents.textureId)
        drawFramebufferQuad()
        GLES30.glBindFramebuffer(GLES30.GL_FRAMEBUFFER, 0)
